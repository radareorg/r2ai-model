...
GROWTH_EXPERIMENT_PLANS = [
    ...
    {
        "id": "workflow.info.elf_overview",
        "topic": "info.elf-overview",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["i", "iI", "ia", "ie", "iM", "ig", "ih", "iH", "it", "iS sha256"],
        "checks": [{"type": "contains", "value": "ELF"}, {"type": "contains", "value": "sha256"}],
        "question": "How can I collect a first, evidence-based ELF inventory with radare2's `i` commands?",
        "answer": "Start with `i` for the open file and IO summary, then use `iI` for loader metadata and mitigation fields. `ia` lists architectures found by the loader; `ie` lists entrypoints and constructors; `iM` reports the main address; and `ig` estimates the program size. Use `ih` for the raw binary-header view and `iH` for named header fields. `it` hashes the complete file, whereas `iS sha256` adds a hash column for each section. Treat the addresses, architectures, mitigations, and hashes printed by the active loader as the evidence for the next step.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_info.inc.c", "libr/bin/p/bin_elf.c"],
        "tags": ["workflow", "info", "elf", "headers", "hashes", "triage"],
    },
    {
        "id": "workflow.info.elf_linkage",
        "topic": "info.elf-linkage",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["ii", "iic", "iE", "il", "ir", "is"],
        "checks": [{"type": "contains", "value": "puts"}, {"type": "contains", "value": "libc.so.6"}, {"type": "contains", "value": "main"}],
        "question": "How do radare2 `i` commands distinguish imported APIs, libraries, relocations, exports, and symbols in an ELF?",
        "answer": "Use `ii` for imported symbols and `iic` to group those imports by behavioral class. `il` reports linked libraries, while `ir` shows relocation records that bind addresses or imported names. `iE` lists exported global symbols, and `is` lists the broader symbol table, including locals, sections, imports, and globals. Do not treat an import as an export or a symbol-table row as proof that code calls it; confirm call sites separately with analysis and xrefs.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_info.inc.c", "libr/bin/p/bin_elf.c"],
        "tags": ["workflow", "info", "elf", "imports", "exports", "relocations", "symbols"],
    },
    {
        "id": "workflow.info.elf_strings",
        "topic": "info.elf-strings",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["izq", "izzq", "it", "iS sha256"],
        "checks": [{"type": "contains", "value": "Hello"}, {"type": "contains", "value": "sha256"}],
        "question": "When should a radare2 workflow use `iz` versus `izz`, and how can it preserve integrity evidence?",
        "answer": "Use `iz` when you want strings from loader-recognized data sections, and use `izz` when the question requires a scan of the whole binary. The `q` suffix makes each result compact for pipelines; `j` is preferable when a consumer needs structured JSON. Preserve context by recording `it` whole-file hashes and, when section-level comparison matters, `iS sha256`. A whole-file string hit can come from padding, metadata, or code, so seek to its address and verify the surrounding section before interpreting it.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_info.inc.c", "libr/bin/p/bin_elf.c"],
        "tags": ["workflow", "info", "elf", "strings", "hashes", "triage"],
    },
    {
        "id": "workflow.info.dwarf",
        "topic": "info.debug-metadata",
        "fixture": "test/bins/elf/dwarf/hello-dwarf2",
        "commands": ["id", "idx", "idl"],
        "checks": [{"type": "contains", "value": "DW_TAG_compile_unit"}, {"type": "contains", "value": "hello.c"}],
        "question": "How should I use radare2 `i` commands to inspect DWARF source metadata without assuming external debug files exist?",
        "answer": "Run `id` to inspect available DWARF debug information and source-line data, and use `idx` to list the source files represented in that metadata. `idl` reports a debug-link filename when the binary carries one; it may be empty when no external debug-link record exists. Source paths and lines are useful navigation evidence, but they are compiler metadata, so validate the mapped address against disassembly before making a code claim.",
        "source_refs": ["test/bins/elf/dwarf/hello-dwarf2", "libr/core/cmd_info.inc.c", "libr/bin/p/bin_elf.c"],
        "tags": ["workflow", "info", "dwarf", "debug", "source-lines"],
    },
    {
        "id": "workflow.info.pe_resources",
        "topic": "info.pe-resources",
        "fixture": "test/bins/pe/manifest.exe",
        "commands": ["iI", "ih", "iH", "iR", "iv"],
        "checks": [{"type": "contains", "value": "Resource 0"}, {"type": "contains", "value": "VS_VERSIONINFO"}],
        "question": "How can radare2 examine PE headers, resources, and version metadata with the `i` family?",
        "answer": "Use `iI` for PE loader and mitigation metadata, `ih` and `iH` to inspect the header representations, `iR` to enumerate resource records, and `iv` for version information. Resource names, types, language IDs, and version fields are binary metadata rather than verified program behavior. Use their printed addresses or offsets to inspect the payload and keep malformed or absent resource output format-specific rather than treating it as an error in every binary.",
        "source_refs": ["test/bins/pe/manifest.exe", "libr/core/cmd_info.inc.c", "libr/bin/p/bin_pe.c"],
        "tags": ["workflow", "info", "pe", "headers", "resources", "version"],
    },
    {
        "id": "workflow.info.java_classes",
        "topic": "info.java-classes",
        "fixture": "test/bins/java/TryWithResources.class",
        "commands": ["iI", "ic", "is", "ii"],
        "checks": [{"type": "contains", "value": "TryWithResources"}, {"type": "contains", "value": "main"}],
        "question": "How do I examine classes, methods, fields, symbols, and imports in a Java class with radare2?",
        "answer": "Confirm the loader result with `iI`, then use `ic` to display class, method, and field metadata. Use `is` for the symbol-oriented view and `ii` for imported or referenced symbols. `icj`, `isj`, and `iij` provide JSON equivalents for automation. Class-file metadata can contain synthetic methods and loader-generated names, so pair this inventory with disassembly or decompilation before assigning behavior to a method.",
        "source_refs": ["test/bins/java/TryWithResources.class", "libr/core/cmd_info.inc.c", "libr/bin/p/bin_java.c"],
        "tags": ["workflow", "info", "java", "classes", "methods", "symbols", "imports"],
    },
    {
        "id": "workflow.info.macho_signatures",
        "topic": "info.macho-signatures",
        "fixture": "test/bins/mach0/MASTestApp-2",
        "commands": ["iI", "iC", "iT"],
        "checks": [{"type": "contains", "value": "application-identifier"}, {"type": "contains", "value": "get-task-allow"}],
        "question": "How should radare2 inspect Mach-O entitlements and code-signature metadata with `iC` and `iT`?",
        "answer": "Use `iI` to confirm that the active loader recognized a Mach-O, then use `iC` to display signature-related information such as entitlements and `iT` to inspect the file-signature data. Treat entitlement keys and their values as observed signing metadata; they do not alone prove runtime enforcement, provenance, or exploitability. On ELF, PE, unsigned, or unsupported files these commands can legitimately produce no corresponding data.",
        "source_refs": ["test/bins/mach0/MASTestApp-2", "libr/core/cmd_info.inc.c", "libr/bin/p/bin_mach0.c"],
        "tags": ["workflow", "info", "macho", "entitlements", "signatures", "ios"],
    },
    {
        "id": "workflow.info.demangle_plugins",
        "topic": "info.demangling-plugins",
        "fixture": "--",
        "commands": ["iD cxx _Z3foov", "iL elf64"],
        "checks": [{"type": "contains", "value": "foo()"}, {"type": "contains", "value": "Executable Link Format"}],
        "question": "How can I demangle a known C++ symbol and check the active binary-loader plugin without opening a target?",
        "answer": "Use `iD cxx _Z3foov` to request C++ demangling for one symbol, and use `iL elf64` to inspect the matching loaded RBin plugin. Demangling is a presentation aid: retain the original mangled name when using the result in scripts or cross-tool comparisons. Plugin inventory is local-build evidence, so do not assume every radare2 installation exposes the same loader set or plugin metadata.",
        "source_refs": ["libr/core/cmd_info.inc.c", "libr/bin/p/bin_elf.c"],
        "tags": ["workflow", "info", "demangle", "plugins", "rbin"],
    },
    {
        "id": "workflow.af.function_discovery",
        "topic": "analysis.function-discovery",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["aa", "af @ entry0", "afe", "afr @ main", "afl~main", "afo main", "afd @ main", "afM"],
        "checks": [{"type": "contains", "value": "main"}, {"type": "contains", "value": "bytes assigned to a function"}],
        "question": "How do I use radare2's `af` family to discover, resolve, and summarize functions in an ELF binary?",
        "answer": "Begin with an analysis pass such as `aa`, define a function at a known entry with `af @ entry0`, use `afe` to analyze entrypoints, and use `afr @ main` only when recursive analysis from that function is appropriate. `afl` lists discovered functions; `afo name` resolves a function name to its address; `afd` gives the function name and offset delta at an address; and `afM` visualizes function coverage. Discovery is heuristic, so verify each important boundary and name with disassembly before using it as ground truth.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_anal.inc.c"],
        "tags": ["workflow", "analysis", "functions", "discovery", "elf"],
    },
    {
        "id": "workflow.af.control_flow",
        "topic": "analysis.function-control-flow",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["aaa", "afi @ main", "afb @ main", "afbj @ main", "afCc @ main", "afbr @ main", "afx @ main", "aflx @ sym.imp.strlen"],
        "checks": [{"type": "contains", "value": "cyclomatic-complexity: 2"}, {"type": "contains", "value": "call sym.imp.strlen"}, {"type": "contains", "value": "\"addr\":"}],
        "question": "How should I inspect a function's basic blocks, complexity, exits, outgoing references, and callers with `af` commands?",
        "answer": "After analysis, use `afi @ main` for function metadata, `afb @ main` for a basic-block listing, and `afbj @ main` for structured block data. `afCc` calculates cyclomatic complexity, while `afbr` reports instructions that leave the function. `afx @ main` reports references used by the current function; `aflx @ sym.imp.strlen` instead asks who references the selected target. Treat complexity and xref counts as navigation signals, not proof of security impact or complete control flow.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_anal.inc.c"],
        "tags": ["workflow", "analysis", "functions", "basic-blocks", "complexity", "xrefs"],
    },
    {
        "id": "workflow.af.types_variables",
        "topic": "analysis.function-types-variables",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["aaa", "afc @ main", "afcf @ main", "afa @ main", "aft @ main", "afv @ main", "afva @ main", "afvx @ main"],
        "checks": [{"type": "contains", "value": "amd64"}, {"type": "contains", "value": "int main"}, {"type": "contains", "value": "dest"}],
        "question": "How can I refine function calling-convention, prototype, argument, type, and variable information with radare2's `af` family?",
        "answer": "Use `afc` to inspect or set the calling convention and `afcf` to print the recovered function prototype. `afa` analyzes call arguments, `aft` performs type-matching analysis, `afv` lists known function variables, `afva` attempts argument/local-variable recovery, and `afvx` reports variable read/write xrefs. These are analysis inferences that may change as types and control flow improve; check them against instructions and ABI rules before relying on an inferred type or variable name.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_anal.inc.c"],
        "tags": ["workflow", "analysis", "functions", "types", "variables", "calling-convention"],
    },
    {
        "id": "workflow.af.reversible_edits",
        "topic": "analysis.function-edits",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["aaa", "afn model_main @ main", "afs int model_main(void) @ model_main", "afS 32 @ model_main", "afi @ model_main", "af- @ model_main", "af model_main @ main", "afi @ model_main"],
        "checks": [{"type": "contains", "value": "name: model_main"}, {"type": "contains", "value": "stackframe: 32"}],
        "question": "How can I safely correct a function name, signature, and stack-frame size, then discard and recreate the analysis?",
        "answer": "Within a disposable radare2 analysis session, use `afn` to rename a function, `afs` to set its signature, and `afS` to set the stack-frame size. Inspect the result with `afi`. `af- @ function` removes the function-analysis record, and `af name @ address` recreates one at the selected address. These commands change radare2's analysis database, not the original executable, but save projects only after checking that manual metadata is justified by the code and ABI.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_anal.inc.c"],
        "tags": ["workflow", "analysis", "functions", "rename", "signature", "stack", "reversible"],
    },
    {
        "id": "workflow.af.bits-boundary",
        "topic": "analysis.function-bits-boundary",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["aaa", "afB 32 @ main", "afi @ main", "afB 64 @ main", "afu 0x847 @ main", "afi @ main"],
        "checks": [{"type": "contains", "value": "bits: 32"}, {"type": "contains", "value": "bits: 64"}],
        "question": "How do `afB` and `afu` adjust function decoding and boundaries without modifying the binary?",
        "answer": "`afB bits` sets the architecture bit width used for the selected function, and `afu end-address` resizes and analyzes a function from its start through the supplied endpoint. Both alter analysis metadata only. Use them when loader metadata, mode switches, or validated control flow justify the change; restore the correct bit width before interpreting output, and confirm the resulting boundary with disassembly and basic-block edges.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_anal.inc.c"],
        "tags": ["workflow", "analysis", "functions", "bits", "boundary", "disassembly"],
    },
    {
        "id": "workflow.af.references-stats",
        "topic": "analysis.function-references-stats",
        "fixture": "test/bins/elf/hello_world",
        "commands": ["aaa", "aflx @ sym.imp.strlen", "afix @ main", "afis @ main"],
        "checks": [{"type": "contains", "value": "0x000007aa"}, {"type": "contains", "value": "call-refs:"}, {"type": "contains", "value": "call"}],
        "question": "How can I distinguish inbound function xrefs, a function's reference summary, and opcode statistics with `aflx`, `afix`, and `afis`?",
        "answer": "Use `aflx @ target` to list functions that reference or call a selected target, `afix @ function` for the selected function's reference summary, and `afis @ function` for opcode and metadata statistics. These views answer different questions: callers of a target, references associated with one function, and the instruction mix of that function. Follow the reported addresses with disassembly or `axt`/`axf` before claiming a reachable call path.",
        "source_refs": ["test/bins/elf/hello_world", "libr/core/cmd_anal.inc.c"],
        "tags": ["workflow", "analysis", "functions", "xrefs", "statistics", "calls"],
    },
]
